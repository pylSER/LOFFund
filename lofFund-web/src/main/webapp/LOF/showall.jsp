...
<div id="loading" style="position: absolute;background-color: white;opacity: 0.8;left: 0px;top: 0px;z-index: 1500;height: 110%;width: 100%;text-align: center;display: none;justify-content: center">
...
for(String code:codelist){
	...
	if(profitBooleans.get(i)){
		if(i==0){
			output=builder.buildRow(firstname, code, arr[0], arr[1], ldvo.getAsk1(), predict,counter);
			
		}
		
		else{
			output=builder.buildRow(ldvo.getName(), code, arr[0], arr[1], ldvo.getAsk1(), predict,counter);
		...
		if(i==0){
			output=builder.buildRowForBad(firstname, code, arr[0], arr[1], ldvo.getAsk1(), predict,counter);
		}
		
		else{
			output=builder.buildRowForBad(ldvo.getName(), code, arr[0], arr[1], ldvo.getAsk1(), predict,counter);
    ...
    $(function() {
        ...
        $("#demo_pag1").bs_pagination({
            ...
            onChangePage: function(event,data) { // returns page_num and rows_per_page after a link has clicked
            	...
            	changePageForAll(data.currentPage);
